...
const menuItems = [
  ...
  { id: 'Species', label: 'Espécies da região', icon: 'eye-outline' },
    ...
    Animated.sequence([
      ...
      setTimeout(() => {
        Animated.timing(dotsOpacity, { toValue: 0, duration: 300, useNativeDriver: true }).start(() => {
          Animated.stagger(80, itemAnims.map(item =>
            Animated.parallel([
    ...
    <View style={styles.container}>
      ...
      <View style={styles.menu}>
        {menuItems.map((item, index) => (
          <Animated.View
            key={item.id}
            style={{ opacity: itemAnims[index].opacity, transform: [{ translateY: itemAnims[index].translateY }] }}
          >
            <TouchableOpacity
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: colors.background, alignItems: 'center', paddingTop: 72, paddingHorizontal: 24, paddingBottom: 32 },
  logoContainer: { marginBottom: 8 },
  logo: { width: 100, height: 100 },
  title: { fontSize: 26, fontWeight: 'bold', color: colors.primary, letterSpacing: 8, marginBottom: 6 },
  subtitle: { fontSize: 11, color: colors.textSecondary, textAlign: 'center', lineHeight: 18, marginBottom: 16 },
  dots: { flexDirection: 'row', gap: 6, marginBottom: 20 },
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.primary },
  menu: { width: '100%', gap: 8 },
  menuItem: { backgroundColor: colors.card, borderRadius: 12, borderWidth: 0.5, borderColor: colors.border, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12 },
  menuIcon: { width: 32, height: 32, borderRadius: 8, backgroundColor: colors.cardSecondary, alignItems: 'center', justifyContent: 'center' },
